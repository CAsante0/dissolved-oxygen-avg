...
run_analysis <- function(context) {
  ...
  # --- FILE DISCOVERY ---
  # Corrected pattern to safely match CSV files without invalid wildcards
  ...
  # --- FILENAME PARSING ---
  # Updated regex and substring logic to cleanly parse YYYYMMDD formats
  file_registry <- data.frame(full_path = all_files) %>%
    mutate(
      file_name = basename(full_path),
      date_string = str_extract(file_name, "\\d{8}"), # Extracts 8 consecutive digits (YYYYMMDD)
      month_id = paste0(substr(date_string, 1, 4), "-", substr(date_string, 5, 6)) # Transforms YYYYMMDD to YYYY-MM
    ) %>%
    filter(!is.na(date_string) & nchar(date_string) == 8)
